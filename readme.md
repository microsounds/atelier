...
### Fingerprint-based SSH authentication with `ssh-agent`
> **WARNING** <br/>
...
Android's [hardware-backed keystore](https://developer.android.com/privacy-and-security/keystore) to retrieve an SSH key passphrase using the fingerprint lock on your Android device.
...
Specifically, the passphrase is generated by using your `~/.ssh/id_rsa.pub` as a nonce value and signing it with a hardware-backed key named `default`, an RSA 4096-bit key generated during post-install.

Some important things to note:
1. The nonce value is not significant, it's simply used to produce a deterministic passphrase that requires your fingerprint to unlock.
2. The key material locked in the hardware-backed keystore is also not important, you are simply using it to generate passphrases for your existing portable SSH keys.

To take advantage of fingerprint-based SSH authentication, you should use the output of `termux-ssh-askpass` as a new passphrase in `ssh-keygen`,
